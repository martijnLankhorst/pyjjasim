...
class EmbeddedGraph:
    ...
    def __init__(self, x, y, node1, node2, require_single_component=False,
        ...
        self.assert_edges_correct_shape()
        self.E = len(self.node1)
        self.assert_edges_contain_existing_nodes()
        self.assert_no_self_loop()

        self.edge_sorter = np.arange(self.edge_count())
        if not _edges_are_sorted:
            self._sort_edges(node1, node2)

        self.assert_nonsimple()
        ...
        if require_single_component:
            self.assert_single_component()
        if require_planar_embedding:
            self.assert_planar_embedding()
    ...
    def add_nodes(self, x, y):
        # if in_place:
        #     self.x = np.append(self.x, np.array(x, dtype=np.double).ravel())
        #     self.y = np.append(self.y, np.array(y, dtype=np.double).ravel())
        #     self.N = len(self.x)
    ...
    def add_edges(self, node1, node2, require_single_component=False, require_planar_embedding=False):
        ...
        #     self.node1 = np.append(self.node1, np.array(node1, dtype=int).ravel())
        #     self.node2 = np.append(self.node2, np.array(node2, dtype=int).ravel())
        #     self.E = len(self.node1)
        #     self._sort_edges(self.node1, self.node2)
        #     self.assert_edges_correct_shape()
        #     self.assert_edges_contain_existing_nodes()
        #     self.assert_no_self_loop()
        #     self.assert_nonsimple()
        #     self._assign_edge_v_array()
        #     if require_single_component:
        #         self.assert_single_component()
        #     if require_planar_embedding:
        #         self.assert_planar_embedding()
    ...
    def remove_nodes(self, node_ids, require_single_component=False, require_planar_embedding=False):
        ...
        #     self._reset_precomputed_quantities()
        #
        #     edge_map = np.ones(self.edge_count(), dtype=int)
        #     edge_map[edge_ids] = 0
        #     edge_map = np.cumsum(edge_map) - edge_map
        #
        #     self.x = np.delete(self.x, node_ids)
        #     self.y = np.delete(self.y, node_ids)
        #     self.N = len(self.x)
        #     self.node1 = n1[edge_ids]
        #     self.node2 = n2[edge_ids]
        #     self.edge_sorter = edge_map[np.delete(self.edge_sorter, np.argsort(self.edge_sorter)[edge_ids])]
        #     self.E = len(self.node1)
        #
        #     self._assign_edge_v_array()
        #     if require_single_component:
    ...
    def remove_edges_by_ids(self, edge_ids, require_single_component=False, require_planar_embedding=False):
        ...
        #     edge_map = np.cumsum(edge_map) - edge_map
        #
        #     self.node1, self.node2 = np.delete(self.node1, edge_ids), np.delete(self.node2, edge_ids)
        #     self.edge_sorter = edge_map[np.delete(self.edge_sorter, np.argsort(self.edge_sorter)[edge_ids])]
        #
        #     self.E = len(self.node1)
        #     self._sort_edges(self.node1, self.node2)
    ...
    def get_faces(self, include_boundary_faces=True, to_list=False):
    ...
    def cycle_space(self, include_boundary_faces=False):
        face_edges, face_lengths = self.get_faces(include_boundary_faces=include_boundary_faces)
